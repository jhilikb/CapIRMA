...
def load_b(args):
    ...
    print('===> Loading Building training datasets')
    # MNIST dataset
    training_set = datasets.BUILDING(
        './data', train=True, download=False, transform=data_transform)

    # Input pipeline
    training_data_loader = DataLoader(
        training_set, batch_size=args.batch_size, shuffle=True, **kwargs)

    print('===> Loading Building testing datasets')
    testing_set = datasets.BUILDING(
        './data', train=False, download=False, transform=data_transform)
    print(testing_set)
    testing_data_loader = DataLoader(
        testing_set, batch_size=args.test_batch_size, shuffle=True, **kwargs)
...
def accuracy(args,output, target, cuda_enabled=True):
    """
    Compute accuracy.
    ...
    assert softmax_v.size() == torch.Size([batch_size, args.num_classes, 1, 1])
